# R Script to plot model.1 vs MLS ch4 data 
...
mls.ch4 <- get.var.ncdf(nc0, "tr14") # this is the setting component
mls.ch4 <- apply(mls.ch4, c(1,2), mean)
...
ch4.zm <- apply(newvv, c(1,2), mean)*(conv/mm.ch4)
rm(newvv)
rm(var)
...
zmin <- 0.0
zmax <- 2.0
ch4.zm[ch4.zm>=zmax] <- zmax
ch4.zm[ch4.zm<=zmin] <- zmin
# ###################################################################################################################################
pdf(file=paste(out.dir,mod1.name,"_CH4_Zonal_mean.pdf", sep=""),width=8,height=6,paper="special",onefile=TRUE,pointsize=12)

# overplot the data 
filled.contour(lat, log.z, ch4.zm, ylab="Altitude (hPa)", xlab="Latitude (degrees)", main=bquote(paste( "HALOE - ", .(mod1.name), ~ CH[4], " comparison", sep=" ")) , zlim=c(zmin,2), col=col.cols(21),
xaxt="n", yaxt="n",key.title = title("ppm"),
ylim=c(log.z[1],log.z[42]), plot.axes= {
contour(lat, log.z, ch4.zm, method = "edge", labcex = 1, col = "gray", cex=0.7, add = TRUE, lty=1, levels=seq(0,2,0.1)) 
contour(mls.lat, mls.hgt, mls.ch4, method = "edge", labcex = 1, col = "black", add = TRUE, lty=1, levels=seq(0,2,0.1)) # obs
...
par(xpd=T)
text(-70,180, paste("Min =",sprintf("%1.3g", min(ch4.zm, na.rm=T) ), "Max =", sprintf("%1.3g", max(ch4.zm, na.rm=T) ), sep=" ") )
par(xpd=F)
