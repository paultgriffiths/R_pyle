# R Script to plot model.1 vs MLS no2 data 
...
mls.no2 <- get.var.ncdf(nc0, "tr06") # this is the setting component
mls.no2 <- apply(mls.no2, c(1,2), mean)
...
no2.zm <- apply(newvv, c(1,2), mean)*(conv/mm.no2)
rm(newvv)
rm(var)
...
# overplot the data 
filled.contour(lat, log.z, no2.zm, ylab="Altitude (hPa)", xlab="Latitude (degrees)", main=paste("MLS - UKCA",mod1.name,"NO2 comparison",sep=" "), zlim=c(zmin,11), col=col.cols(22),
xaxt="n", yaxt="n",key.title = title("ppb"),
ylim=c(log.z[1],log.z[42]), plot.axes= {
contour(lat, log.z, no2.zm, method = "edge", labcex = 1, col = "gray", cex=0.7, add = TRUE, lty=1, levels=seq(0,10,1)) 
contour(mls.lat, mls.hgt, mls.no2, method = "edge", labcex = 1, col = "black", add = TRUE, lty=1, levels=seq(0,10,1)) # obs
...
par(xpd=T)
text(-70,180, paste("Min =",sprintf("%1.3g", min(no2.zm, na.rm=T) ), "Max =", sprintf("%1.3g", max(no2.zm, na.rm=T) ), sep=" ") )
par(xpd=F)
