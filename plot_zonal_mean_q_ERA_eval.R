...
# ERA data
nc0 <- open.ncdf(paste(obs.dir, "ERA/data/erai_q_2000-2011_N48_P37.nc", sep=""), readunlim=FALSE)
...
sig.mask <- ifelse( abs( ((ukca.z-era.z)/(era.sd))) > 1.0, TRUE, FALSE)
...
# ###################################################################################################################################
pdf(file=paste(out.dir,mod1.name,"-ERA_Q_pd.pdf", sep=""),width=8,height=6,paper="special",onefile=TRUE,pointsize=12)

# overplot the data 
filled.contour(lat.era, log.z, diff, ylab="Altitude (hPa)", xlab="Latitude (degrees)", main=paste(mod1.name,"- ERA Q bias ",sep=" "), zlim=c(zmin,zmax), col=heat.cols(20), xaxt="n", yaxt="n",
