...
# model (UM) tropopause height - missing from HTAP runs...
...
# need to generate arrays of surface area and volume. Volume = surface area * geop?
source("~/Dropbox/r_scripts/convert_deg_to_meters.R")
gb.sa <- deg2met
vol <- array(NA, dim=c(length(lon), length(lat), length(hgt)) )
for(i in 1:length(hgt)){
  vol[,,i] <- gb.sa*hgt*1E3
}

# using the 150 ppbv O3 mask
(source(paste(script.dir, "calc_ozone_mask.R", sep="")))
...
ho2.no<- get.var.ncdf(nc1,"field1875_1") * mask 
meoo.no<- get.var.ncdf(nc1,"field1875_2") * mask 
isoo.no<- get.var.ncdf(nc1,"field1875_3") * mask 
alkaRO2.no<- get.var.ncdf(nc1,"field1875_4") * mask 
alkeRO2.no<- get.var.ncdf(nc1,"field1875_5") * mask 

# sum the terms
net.p     <- (ho2.no + meoo.no + isoo.no + alkaRO2.no + alkeRO2.no)
...
# deposition fields 
#l1.8 <- get.var.ncdf(nc1,o3.dd.code) * mask *3.0
#l1.9 <- get.var.ncdf(nc1,noy.dd.code) * mask *3.0
#l1.10 <- get.var.ncdf(nc1,noy.wd.code) * mask *3.0
o3.ho2<- get.var.ncdf(nc1,"field1875_6") * mask
o3.oh<- get.var.ncdf(nc1,"field1875_7") * mask
o1d.h2o<- get.var.ncdf(nc1,"field1875_8") * mask
n2o5.h2o<- get.var.ncdf(nc1,"field1875_9") * mask
o3.isop<- get.var.ncdf(nc1,"field1875_10") * mask
o3.apin<- get.var.ncdf(nc1,"field1875_11") * mask
o3.alken<- get.var.ncdf(nc1,"field1875_12") * mask
# deposition fields 
noy.ddep<- get.var.ncdf(nc1,"field1875_13") * mask
o3.ddep<- get.var.ncdf(nc1,"field1875_14") * mask
o3.wdep<- get.var.ncdf(nc1,"field1875_15") * mask

# sum the terms
net.l <- (o3.ho2 + o3.oh + o1d.h2o + n2o5.h2o + o3.isop + o3.apin
          + o3.alken)#(l1.1 + l1.2 + l1.3 + l1.4 + l1.5 + l1.6 + l1.7)
...
dd.map <- (noy.ddep + o3.ddep + o3.wdep)#(l1.8 + l1.9)
dd.map[dd.map==0.00] <- NA
dd.map <- apply(dd.map,c(1,2,4),sum,na.rm=T) # vertically integrate the deposition
...
dd.map <- apply(dd.map.std, c(1,2), mean, na.rm=T)
# ################################################################################
# Write these values on the plot
# calc the total production
o3.prod.yrt = sum(ho2.no + meoo.no + isoo.no + alkaRO2.no + alkeRO2.no)*conv.factor

# calculate the 3d and 2d (deposition) loss 
o3.loss1.yrt = sum(o3.ho2 + o3.oh + o1d.h2o + n2o5.h2o + o3.isop + o3.apin
                   + o3.alken)*conv.factor
o3.loss2.yrt = sum(noy.ddep + o3.ddep + o3.wdep)*conv.factor

o3.netchem <-  o3.prod.yrt - o3.loss1.yrt 
ste.inf    <-  o3.prod.yrt - o3.loss1.yrt - o3.loss2.yrt

# diagnosed STE (moles/grid cell/s)
ste.diag <- get.var.ncdf(nc1, "field671") * mask
