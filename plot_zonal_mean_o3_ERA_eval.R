...
o3.zm <- apply(newvv, c(1,2), mean)*(conv/mm.o3)
rm(newvv)
rm(var)
...
# ###################################################################################################################################
pdf(file=paste(out.dir,mod1.name,"_O3_Zonal_mean.pdf", sep=""),width=8,height=6,paper="special",onefile=TRUE,pointsize=12)

# overplot the data 
filled.contour(lat, log.z, o3.zm, ylab="Altitude (hPa)", xlab="Latitude (degrees)", main=bquote(paste( "HALOE - ", .(mod1.name), ~ O[3], " comparison", sep=" ")), zlim=c(zmin,11), col=col.cols(22),
xaxt="n", yaxt="n",key.title = title("ppm"),
ylim=c(log.z[1],log.z[42]), plot.axes= {
contour(lat, log.z, o3.zm, method = "edge", labcex = 1, col = "gray", cex=0.7, add = TRUE, lty=1, levels=seq(0.5,11.5,1)) 
...
par(xpd=T)
text(-70,180, paste("Min =",sprintf("%1.3g", min(o3.zm, na.rm=T) ), "Max =", sprintf("%1.3g", max(o3.zm, na.rm=T) ), sep=" ") )
par(xpd=F)
