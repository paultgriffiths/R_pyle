# R Script to plot model.1 vs MLS ClO data 
...
# extract the MLS Obs
nc0 <- open.ncdf(paste(obs.dir, "MLS/mls_clo_2005-2011.nc", sep=""))
...
clo.zm <- apply(newvv, c(1,2), mean)*(conv/mm.clo)
rm(newvv)
rm(var)
...
# overplot the data 
filled.contour(lat, log.z, clo.zm, ylab="Altitude (hPa)", xlab="Latitude (degrees)", main=paste("MLS - ",mod1.name,"ClO comparison",sep=" "), zlim=c(zmin,0.6), col=col.cols(12),
xaxt="n", yaxt="n",key.title = title("ppb"),
ylim=c(log.z[1],log.z[37]), plot.axes= {
contour(lat, log.z, clo.zm, method = "edge", labcex = 1, col = "gray", cex=0.7, add = TRUE, lty=1, levels=seq(0,0.5,0.05)) 
...
par(xpd=T)
text(-70,135, paste("Min =",sprintf("%1.3g", min(clo.zm, na.rm=T) ), "Max =", sprintf("%1.3g", max(clo.zm, na.rm=T) ), sep=" ") )
par(xpd=F)
